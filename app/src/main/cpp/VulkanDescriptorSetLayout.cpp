                                                     ...
                                                     : device(device), count(count)
{
	...
	pool = createDescriptorPool(device, descriptorPoolSizes.data(), count);

	set = createDescriptorSets(device, pool, layout);
...
VulkanDescriptorSetLayout::VulkanDescriptorSetLayout(VulkanDescriptorSetLayout&& other) noexcept
	...
	pool(std::exchange(other.pool, (VkDescriptorPool)VK_NULL_HANDLE)),
    set(std::exchange(other.set, (VkDescriptorSet)VK_NULL_HANDLE)),
...
VkDescriptorPool VulkanDescriptorSetLayout::createDescriptorPool(VkDevice device, VkDescriptorPoolSize* descriptorPoolSizes,
																 uint32_t count)
{
...
VkDescriptorSet VulkanDescriptorSetLayout::createDescriptorSets(VkDevice device, VkDescriptorPool pool, VkDescriptorSetLayout setLayout)
{
...
void VulkanDescriptorSetLayout::destroyDescriptorPool(VkDevice device, VkDescriptorPool* pool)
{
